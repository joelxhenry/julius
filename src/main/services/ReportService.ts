...
export class ReportService {
  ...
  async getInventoryValuation(params: InventoryValuationParams): Promise<InventoryValuationResult> {
    const { category, lowStockOnly } = params;

    const conditions = [];
    if (category) conditions.push(eq(schema.inventory.category, category));
    if (lowStockOnly) {
      conditions.push(
        sql`${schema.inventory.quantity} < ${schema.inventory.minLevel}`
      );
    }
    ...
    const items = await this.db
      ...
      .orderBy(schema.inventory.sku);

    const result: InventoryValuationItem[] = items.map((item) => {
    ...
    const totalCostValue = result.reduce((sum, i) => sum + i.totalCost, 0);
    const totalRetailValue = result.reduce((sum, i) => sum + i.totalRetail, 0);
  ...
  async getSalespersonPerformance(params: DateRangeParams): Promise<SalespersonPerformanceResult> {
    const { startDate, endDate } = params;
    ...
    for (const emp of salespeople) {
      // Invoice aggregation
      const invConds = [eq(schema.invoices.salespersonId, emp.id)];
      if (startDate) invConds.push(gte(schema.invoices.invDate, startDate));
      if (endDate) invConds.push(lte(schema.invoices.invDate, endDate));

      const invResult = await this.db
        .select({
          count: count(),
          total: sql<string>`COALESCE(SUM(CAST(${schema.invoices.total} AS numeric)), 0)`,
        })
        .from(schema.invoices)
        .where(and(...invConds));

      // Quotation aggregation
      const quoteConds = [eq(schema.quotations.salespersonId, emp.id)];
      if (startDate) quoteConds.push(gte(schema.quotations.quoteDate, startDate));
      if (endDate) quoteConds.push(lte(schema.quotations.quoteDate, endDate));

      const quoteResult = await this.db
        .select({
          count: count(),
          total: sql<string>`COALESCE(SUM(CAST(${schema.quotations.total} AS numeric)), 0)`,
        })
        .from(schema.quotations)
        .where(and(...quoteConds));

      // Credit note aggregation
      const cnConds = [eq(schema.creditNotes.salespersonId, emp.id)];
      if (startDate) cnConds.push(gte(schema.creditNotes.crDate, startDate));
      if (endDate) cnConds.push(lte(schema.creditNotes.crDate, endDate));

      const cnResult = await this.db
        .select({
          count: count(),
          total: sql<string>`COALESCE(SUM(CAST(${schema.creditNotes.total} AS numeric)), 0)`,
        })
        .from(schema.creditNotes)
        .where(and(...cnConds));

      const invoiceTotal = Number(invResult[0]?.total ?? 0);
      const creditNoteTotal = Number(cnResult[0]?.total ?? 0);

      rows.push({
        employeeId: emp.id,
        name: [emp.firstName, emp.lastName].filter(Boolean).join(' ') || emp.code,
        invoiceCount: Number(invResult[0]?.count ?? 0),
        invoiceTotal,
        quoteCount: Number(quoteResult[0]?.count ?? 0),
        quoteTotal: Number(quoteResult[0]?.total ?? 0),
        creditNoteCount: Number(cnResult[0]?.count ?? 0),
        creditNoteTotal,
        netSales: invoiceTotal - creditNoteTotal,
      });
    }
    ...
    const totals = rows.reduce(
      (acc, r) => ({
        ...
        quoteCount: acc.quoteCount + r.quoteCount,
        quoteTotal: acc.quoteTotal + r.quoteTotal,
