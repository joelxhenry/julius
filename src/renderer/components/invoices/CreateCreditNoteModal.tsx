...
export function CreateCreditNoteModal({
  ...
  const isPartialPayment = parseFloat(invoice.totalPaid) < parseFloat(invoice.total);
  const exceedsPaymentCap = totals.total > maxCreditAllowed + 0.001;
  ...
  return (
    ...
    >
      <Stack gap="md">
        ...
        <Alert icon={<IconAlertCircle size={16} />} color={isPartialPayment ? 'yellow' : 'blue'} variant="light" p="xs">
          <Text size="xs">
            Select items and quantities to credit. The invoice will be updated accordingly and the credit note funds will be available to apply to future payments.
            {isPartialPayment && (
              <Text span fw={600}> Max credit: {formatCurrency(maxCreditAllowed)} (limited to amount paid).</Text>
            )}
          </Text>
        </Alert>
        ...
        <Group justify="space-between" mt="xs">
          ...
          <Group gap="sm">
            ...
            <Button
              ...
              disabled={totals.total <= 0 || exceedsPaymentCap}
              color="green"
            >
              Issue Credit Note
            </Button>
