...
export function ReceivingTab({ sku }: ReceivingTabProps) {
  ...
  const loadReceiving = useCallback(async () => {
    setLoading(true);
    try {
      const result = await window.electron.invoke(IpcChannel.GET_INVENTORY_RECEIVING_BY_SKU, { sku });
  ...
  const groups = useMemo<ReceivingGroup[]>(() => {
    ...
    return Array.from(map.entries())
      .map(([key, records]) => ({
        ...
        supplier: records.find((r) => r.supplier)?.supplier ?? null,
        supplierId: records.find((r) => r.supplierId)?.supplierId ?? null,
    ...
    async (group: ReceivingGroup, format: ExportFormat) => {
      ...
      try {
        const columns: ExportColumn[] = [
          ...
          { header: 'Supplier', key: 'supplier', format: 'text' },
        ...
        const request: ExportRequest = {
          fileName: `Receiving ${sku} ${refLabel}`,
          format,
          columns,
          rows,
          sheetName: 'Receiving',
  ...
  const renderSupplierLink = (group: ReceivingGroup) => {
    if (!group.supplier) return null;
    if (!group.supplierId) {
      return (
    ...
    return (
      <Tooltip label="Open supplier in new tab" withArrow position="top">
        <Anchor
          component="button"
          type="button"
          size="sm"
          onClick={() => openTab(`/suppliers/${group.supplierId}`)}
  ...
  // Columns for the "View" dialog table; reference/supplier live in the header.
  ...
  return (
    <Stack gap="md">
      ...
      {loading ? (
        ...
        <Paper p="xl" radius="md" withBorder>
          <Text c="dimmed" ta="center">
            No receiving records found for this item
          </Text>
        </Paper>
      ) : (
        <>
      ...
      <Modal
        ...
        size="xl"
        title={
          viewGroup?.reference ? `Reference #${viewGroup.reference}` : 'Receiving Records'
        }
      >
        {viewGroup && (
          <Stack gap="md">
            <Group gap="sm" wrap="wrap">
              {renderSupplierLink(viewGroup)}
              <Badge variant="light" color="green">
